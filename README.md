...
- **关系主网络**（自研 Canvas 渲染 + igraph DrL 预计算布局）：默认显示最大连通分量中的 35,975 人、70,051 条关系，去除与主网络断开的外围分量
- **标签显示滑杆**：6 档（无 / 极少 / 较少 / 中等 / 较多 / 尽量多），按**度中心性**分位数决定标签密度；度数低的人物不显示标签、颜色更淡
...
- **亲缘谱系候选**：从 CBDB `KIN_DATA` 提取直系亲缘连通组，单选后按出生年代从上到下排列，并显示亲缘关系边；候选组可能包含经共同子女相连的姻亲
- **亮色 / 暗色主题**：一键切换并记忆本地偏好
...
| `data/families.json` | 亲缘谱系候选（人物组 + CBDB 亲缘边） |
...
- **默认显示**：为去除与主网络断开的外围散点，前端仅显示最大社会关系连通分量，共 **35,975 人 / 70,051 条关系**；原始数据仍完整保留。
...
3. **配色**：浅色底（`#FAFAFA`）、正文深灰 `#333`；相近朝代使用相近色系，关系边使用与节点区分开的低饱和浅色系

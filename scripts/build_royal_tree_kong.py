...
def main():
 ...
 source_id_map={str(x.get('source_id')):str(x['id']) for x in src['members'] if x.get('source_id') is not None}
 support=[]
 for edge in families.get('edges',[]):
  s,t=str(edge.get('source')),str(edge.get('target'))
  # Keep only support links whose two CBDB endpoints are represented by
  # namespaced Wiki/branch members; never pull raw numeric nodes into family mode.
  if s in source_id_map and t in source_id_map:
   support.append({'source':source_id_map[s],'target':source_id_map[t],'kin_code':edge.get('kin_code'),'generation_gap':edge.get('generation_gap'),'direction':edge.get('direction'),'structural':False,'evidence_type':'CBDB family edge'})
